...
\sjtusetup{
  ...
  info = {%
    ...
    % 标题
    %
    zh / title           = {基于多源时空信息的 GNSS 事件检测方法及其对高精度定位影响的研究},
    en / title           = {Research on GNSS Event Detection Based on Multi-source Spatio-temporal Information and Its Impact on High-precision Positioning},
    %
    % 标题页标题
    %   可使用“\\”命令手动控制换行
    %
    ...
    % 关键词
    %
    zh / keywords        = {GNSS，弹性功率，异常检测，星载设备健康管理，精密单点定位， 差分码偏差},
    en / keywords        = {GNSS, Flex Power, Anomaly Detection, On-board instrument PHM, PPP, DCB},
    %
    % 姓名
    %
    zh / author          = {李\quad{}嘉\quad{}渝},
    en / author          = {JiayuLi},
    %
    % 学号
    %
    ...
    % 指导教师
    %
    zh / supervisor      = {项\quad{}艳\quad{}副研究员},
    en / supervisor      = {Assoc. Prof.\ Yan Xiang},
    %
    % 副指导教师
    %
...
% 将 draft=true 开启即可不编译图片；改成 draft=false 则正常显示
...
\usepackage{ragged2e} % 用于更好的换行对齐

% 如果希望时间段靠左对齐以节省空间，可以使用下面这个
